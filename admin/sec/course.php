<?php
    ...
    // Fetch data
    $sql = "SELECT * FROM course $search_condition ORDER BY id ASC LIMIT $start_from, $limit";
    $result = $conn->query($sql);

    // Total records count
    $count_sql = "SELECT COUNT(*) as total FROM course $search_condition";
    ...
    <div class="card shadow-lg">
        <div class="card-header text-white">
            <div class="d-flex justify-content-between align-items-center">
                <h3 class="mb-0"><i class="fas fa-users-cog me-2"></i>Course Management</h3>
                <div class="d-flex align-items-center">
                    <form class="d-flex search-box" method="get" action="">
                        <div class="input-group">
                            <input type="hidden" name="q" value="course">
                            <input type="text" name="search" class="form-control border-end-0" placeholder="Search course..." value="<?= htmlspecialchars($search) ?>">
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-search"></i>
                            </button>
                            <?php if (!empty($search)): ?>
                                <a href="?q=course" class="btn btn-danger ms-1">
                    ...
                    <nav aria-label="Page navigation">
                        <ul class="pagination mb-0">
                            ...
                            <li class="page-item <?= ($page <= 1) ? 'disabled' : '' ?>">
                                <a class="page-link" href="?q=course&page=1<?= !empty($search) ? '&search='.urlencode($search) : '' ?>" aria-label="First">
                            ...
                            <!-- Previous Page -->
                            <li class="page-item <?= ($page <= 1) ? 'disabled' : '' ?>">
                                <a class="page-link" href="?q=course&page=<?= $page-1 ?><?= !empty($search) ? '&search='.urlencode($search) : '' ?>" aria-label="Previous">
                            ...
                            <?php for ($i = $start_range; $i <= $end_range; $i++): ?>
                                <li class="page-item <?= ($i == $page) ? 'active' : '' ?>">
                                    <a class="page-link" href="?q=course&page=<?= $i ?><?= !empty($search) ? '&search='.urlencode($search) : '' ?>"><?= $i ?></a>
                                </li>
                            <?php endfor; ?>
                            ...
                            <li class="page-item <?= ($page >= $total_pages) ? 'disabled' : '' ?>">
                                <a class="page-link" href="?q=course&page=<?= $page+1 ?><?= !empty($search) ? '&search='.urlencode($search) : '' ?>" aria-label="Next">
                            ...
                            <!-- Last Page -->
                            <li class="page-item <?= ($page >= $total_pages) ? 'disabled' : '' ?>">
                                <a class="page-link" href="?q=course&page=<?= $total_pages ?><?= !empty($search) ? '&search='.urlencode($search) : '' ?>" aria-label="Last">
            ...
            <?php else: ?>
                <div class="text-center py-5">
                    ...
                    <h4 class="text-muted mb-3">
                        <?php if (!empty($search)): ?>
                            No course found matching your search criteria
                        <?php else: ?>
