...
function getDeliveryCharge() {
    ...
    return checked ? parseFloat(checked.getAttribute('amount')) : 0;
}
...
function calculateSummary(discountValue = 0, code = '') {
    ...
    const hasDelivery = document.querySelector('input[name="address_type"]') !== null;
    ...
    if (hasDelivery) {
        ...
        deliveryRow.innerHTML = '<span>Delivery</span><span>৳ ' + delivery.toFixed(2) + '</span>';
    } else {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const deliveryRadios = document.querySelectorAll('input[name="address_type"]');
